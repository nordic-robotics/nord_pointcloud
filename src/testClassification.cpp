...
for(uint k=0;k<308;k++){
 point[0].histogram[k]=centroids->data[0].features.vfh[k];
  ...
  nord_messages::FlannSrv srv;
  srv.request.data.push_back( centroids->data[0].features );
  
  if (client.call(srv)) {
    for (uint i=0;i<srv.response.names.size();i++) {
      std::cout << srv.response.names[i].data << ":   " << srv.response.counts[i] << std::endl;
    }
  }
  else {
    ROS_INFO("FAILED YOU FUCK!");
  }
...
int main (int argc, char** argv){
  ...
  ros::init (argc, argv, "testpclclassifier");
  ros::NodeHandle nh;
  ...
  client = nh.serviceClient<nord_messages::FlannSrv>("/nord/pointcloud/shape_classification_service");
